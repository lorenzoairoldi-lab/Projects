function createQueries(pool) {
  ...
  async function update(userId, id, data) {
    const fields = [];
    const values = [userId, id]; // $1 = user_id, $2 = id
    let idx = 3;
    ...
    const { rows } = await pool.query(
      `UPDATE workouts SET ${fields.join(", ")}, updated_at = NOW() WHERE user_id = $1 AND id = $2 RETURNING *`,
      values
    );
    return rows[0];
  }
  ...
  return { create, list, getById, update, remove, getByUserAndDateRange };
}
