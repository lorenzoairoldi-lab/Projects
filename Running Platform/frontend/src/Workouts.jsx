  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-slate-800">🏃 My Workouts</h2>
        <button
          onClick={() => {
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="card p-6 mb-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              <label className="block text-xs font-medium text-slate-500 mb-1">Elevation (m)</label>
              <input
          ...
          <div className="flex gap-3 mt-5">
            <button
              type="submit"
              disabled={saving}
              className="btn px-6 py-2.5 bg-accent hover:bg-accent/80 disabled:opacity-50 text-white font-medium"
            >
            ...
            {editingId && (
              <button
                type="button"
                onClick={resetForm}
                className="btn px-6 py-2.5 bg-slate-200 hover:bg-slate-300 text-slate-700 font-medium"
              >
                Cancel
              </button>
            )}
      ...
      {loading ? (
        ...
        <div className="text-center py-16 card">
          ...
          <p className="text-slate-500 text-lg font-medium">No workouts yet</p>
          <p className="text-slate-400 text-sm mt-1">Create your first one!</p>
        </div>
      ) : (
        <div className="card overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {workouts.map((w) => (
                  <tr key={w.id} className="border-b border-slate-100 hover:bg-primary/5 transition">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <div className="flex gap-1.5">
                        <button
                          onClick={() => startEdit(w)}
                          className="btn px-3 py-1.5 bg-primary/10 text-primary hover:bg-primary hover:text-white text-xs"
                        >
                          Edit
                        </button>
                        <button
                          onClick={() => handleDelete(w.id)}
                          className="btn px-3 py-1.5 bg-danger/10 text-danger hover:bg-danger hover:text-white text-xs"
                        >
                          Delete
                        </button>
                      </div>
